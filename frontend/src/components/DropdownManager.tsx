...
export default function DropdownManager({ tables = [] }: DropdownManagerProps) {
  ...
  const fetchColumns = async (tableName: string) => {
    try {
      setLoading(true);
      const response = await axios.post(`/fetchcolumn`, {
  ...
  const fetchExistingOptions = async () => {
    try {
      ...
      if (response.data.success && response.data.data) {
        const existingOptions = response.data.data.map((opt: string) => ({
          value: opt,
        }));
  ...
  const handleSaveOptions = async () => {
    try {
      ...
      if (
        existingOptionsResponse.data.success &&
        existingOptionsResponse.data.data
      ) {
        // Keep the existing options for other columns
        existingTableOptions =
          existingOptionsResponse.data.dropdown_options || [];
      }
