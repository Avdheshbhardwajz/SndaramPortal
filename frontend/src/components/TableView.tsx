...
//             hasError: true,
//             message:
//               "Please enter only letters and numbers (no special characters)",
//           };
//         }
...
//   function storeDropdownColumns(
//     tableName: string,
//     configs: { [key: string]: ColumnConfig }
//   ) {
//     try {
//       // Get existing dropdown mappings from localStorage
//       const existingMappings = JSON.parse(
//         localStorage.getItem("dropdownColumns") || "{}"
//       );
...
//         (change) =>
//           change.tableName === tableName && change.status === "pending"
//       );
...
//           if (
//             columnConfigs[field]?.editType === "date calendar" &&
//             params.value
//           ) {
...
//   useEffect(() => {
//     const csvFile = `../assets/csv/${tableName
//       .toLowerCase()
//       .replace(/ /g, "_")}.csv`;
...
//             description:
//               "CSV must contain header row, editability row, and edit type row.",
//           });
//           return;
//         }
...
//           {};
//         try {
//           existingDropdowns = JSON.parse(
//             localStorage.getItem("dropdownColumns") || "{}"
//           );
//         } catch (error) {
//           console.error(
//             "Error reading dropdown options from localStorage:",
//             error
//           );
//         }
...
//             const isDropdown =
//               editType === "dropdown" ||
//               editType === "drop down with predefined value in the column";

//             // Get dropdown options from CSV
//             let dropdownOptions = isDropdown
//               ? Array.from(
//                   new Set(result.data.slice(3).map((row) => row[index]))
//                 ).filter(Boolean)
//               : undefined;
...
//             (key) =>
//               configs[key].editType === "dropdown" ||
//               configs[key].editType ===
//                 "drop down with predefined value in the column"
//           )
//         ) {
...
//               updatedDropdowns[tableName][header] =
//                 config.dropdownOptions || [];
//             });

//           localStorage.setItem(
//             "dropdownColumns",
//             JSON.stringify(updatedDropdowns)
//           );
//         }
...
//             >
//               <SelectTrigger
//                 className={cn(validation?.hasError && "border-red-500")}
//               >
...
//         return (
//           <div>
//             <Input value={value?.toString() || ""} {...commonInputProps} />
//             {renderValidationError()}
//           </div>
//         );
//     }
...
//     const originalValues: Record<string, string | number | boolean | null> = {};
...
//       localStorage.setItem(
//         "pendingChanges",
//         JSON.stringify([...existingChanges, changeRecord])
//       );
...
//         description:
//           "Your edited data will reflect here once approved by checker.",
//       });
//     } catch (error) {
